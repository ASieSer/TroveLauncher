 ...
 *
 *   -> <id> spawn <exe64> <ticket64> <auth64> <reserved64> <wait_ms> [pid,pid...]
 *      (<reserved64> is always empty and ignored)
                      ...
                      const char *exclude_csv)
{
    ...
    /* The two handles above are the only ones the game may inherit. */
    HANDLE handles[2] = {hmap, hevent};
    BOOL attr_ok = UpdateProcThreadAttribute(attr, 0, PROC_THREAD_ATTRIBUTE_HANDLE_LIST,
    ...
    BOOL ok = CreateProcessW(exe_to_run, cmd, NULL, NULL, TRUE,
...
int main(void)
{
    ...
    while (fgets(line, LINE_MAX_BYTES, stdin)) {
        ...
        if (strcmp(cmd, "ping") == 0) {
            ...
            /* The reserved slot is read only to keep the arguments after it
               in place; its contents are ignored. */
            char *reserved = decode_arg(next_token(&cursor));
