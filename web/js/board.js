...
(function () {
    ...
    function appendAccounts(rows, list, groupId) {
        ...
        rows.appendChild((App.accountList || cardGrid)(visible, groupId));
    ...
    /** How the accounts of one group are laid out. Cards, unless a local
     *  module has put something else in App.accountList: it is handed the
     *  accounts to show and the group they belong to, and returns the node to
     *  put on the board. Nothing else about the board changes with it - the
     *  group boxes, the folding and the drag are the same either way. */
    App.accountList = null;

    function cardGrid(visible, groupId) {
    ...
    function groupHead(group, list) {
        ...
        remove.addEventListener('click', async (e) => {
            ...
            if (!await App.confirm(
                `Its accounts are not deleted with it: they move to Ungrouped.`,
    ...
    function buildActions(account, opts) {
        ...
        const stopping = account.status === 'stopping';

        actions.appendChild(App.actionButton('trash', 'Delete account', 'danger', async () => {
            if (!await App.confirm(
                'Its saved session and password are removed from this machine.',
        ...
        if (account.status === 'running') {
            ...
            // A stop while it is still starting up, for the click that was not
            // meant. It takes effect at the next step of the launch, which is
            // seconds, and nothing is marked failed: the account goes back to
            // exactly what it was before.
            const give = App.actionButton('stop',
                stopping
                    ? 'Stopping - it gives up at the next step of the launch'
                    : account.status === 'checking'
                        ? 'Stop checking this account'
                        : 'Stop launching this account',
                'danger', stopping ? null : () => App.cancelLaunch(account),
                { disabled: stopping });
            if (grow) give.classList.add('grow');
    ...
    // Published for a local layout module: the same action buttons, so an
    // alternative arrangement of the accounts does not grow a second copy of
    // the part that changes most.
    App.buildActions = buildActions;
})();
