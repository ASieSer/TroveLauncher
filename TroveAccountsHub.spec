...
Two things worth knowing about this file:
  ...
  * `web/js/local.js` and `web/css/local.css`, if present, are left OUT unless
    the environment has TAH_LOCAL set. They are local additions that are not
    part of what gets published, and `web/` travelling whole would otherwise
    carry them into a release without anyone noticing.
...
# The interface loads web/js/local.js and web/css/local.css if they are there.
# They are local additions, ignored by git, and they stay out of a build unless
# it is asked for them:
#
#     set TAH_LOCAL=1     (or TAH_LOCAL=1 in the environment on Linux)
#
# Off by default on purpose. `web/` travels whole, so without this a release
# built on a machine that has those files would quietly ship them.
if not os.environ.get("TAH_LOCAL"):
    a.datas = [entry for entry in a.datas
               if Path(entry[0]).name not in ("local.js", "local.css")]
