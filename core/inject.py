...
def resolve_game_pid(spawn_pid: int, exe_name: str, *, exclude=(),
    ...
    the session closed after a few seconds. Here we wait for the game's process
    to appear.
...
def spawn(game_exe: Path, ticket: str, auth_server: str, *,
          wait_ms: int = 30000, log=print) -> dict:
    ...
    ok = kernel32.CreateProcessW(
        str(exe_to_run), cast(cmd_buf, wintypes.LPWSTR), None, None, True,
