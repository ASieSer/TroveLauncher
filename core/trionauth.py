...
# What Trion's error codes mean, for the ones that have actually been seen.
# The code arrives in the X-Trionworlds-Error header and on its own it is all
# the user was shown - "LOGIN_FAILED", with nothing to say what to do about it.
# Only codes observed in real answers go here; an unknown one is shown as it
# came rather than dressed up with a guess.
READABLE_ERRORS = {
    "LOGIN_FAILED": (
        "Trion rejected the email and password. If the password was changed "
        "on Trion's site, set the new one on this account with the gear "
        "button. A saved password is only really tested every two days - "
        "in between, the session is renewed without it - so a change can "
        "take that long to show up here."),
}
...
class TrionAuth:
    def __init__(self, *, username: str, password: str, channel: str, user_agent: str,
        ...
        # Whether the last get_ticket sent the password and Trion took it. A
        # cached ticket is reused for two days without the password going
        # anywhere, so "get_ticket returned" says nothing about the password -
        # and saving one on that evidence is how a wrong password was stored
    ...
    def _authenticate(self, token_provider: Callable[[], str] | None) -> str:
        ...
        # Everything Trion said about the refusal, into the log: the status,
        # every header of its own, and the start of the body. Before this the
        # log held the bare code and nothing else to go on. The REQUEST is never
        # logged - it carries the password - and nothing here echoes it.
        said = {k: v for k, v in r.headers.items()
                if k.lower().startswith("x-trionworlds")}
        body = " ".join((r.text or "").split())[:300]
        self._log(f"[auth] refused: HTTP {r.status_code}"
                  + (f" | {said}" if said else "")
                  + (f" | body: {body}" if body else ""))

        code = err or (body[:200] if body else f"HTTP {r.status_code}")
        readable = READABLE_ERRORS.get((err or "").strip().upper())
        # The code stays on the end, so it can still be searched for.
        raise AuthError(f"{readable} ({code})" if readable else code)
    ...
    def get_ticket(self, *, token_provider: Callable[[], str] | None = None) -> str:
        ...
        if age is not None and age < REAUTH_AFTER_SECONDS and self._touch(rec["ticket"]):
            ...
            # Only here did the password actually reach Trion and get accepted.
