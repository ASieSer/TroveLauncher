...
# Everything an account is made of. See the data model above.
ACCOUNT_KEYS = ("email", "name", "color", "group", "region", "flagged")
DEFAULT_REGION = "EU"
...
DEFAULTS = {
    ...
    "theme": {"accent": "#22c55e", "customs": [], "stars": True,
              "font": "system", "club": ""},
    # Background image. Only the FILE NAME is kept, never a path: the image is
    # copied into this same folder when it is chosen, so it keeps working when
    # the original is moved or deleted, and it travels with the rest of the
    # data. `dim` and `blur` are what keep the interface readable over it.
    "wallpaper": {"file": "", "dim": 0.45, "blur": 0},
    # The mark in the top bar, when the user brought their own. Kept the same
    # way as the wallpaper: a copy in this folder, and only its name here.
    # `icon` is whether the title bar and taskbar wear it too.
    "logo": {"file": "", "icon": True},
    # Where the window was when it was last closed. Empty until it has been
    # closed once; see main._remember_window.
    "window": {},
    # The mods pane: whether it lists them without their preview images.
    "mods": {"compact": False},
    # The rail on the left: kept open, or a strip of icons that widens on hover.
...
def load() -> dict:
    ...
    # An account keeps only the keys of the model; anything else an older
    # version wrote is dropped here and gone from the file on the next save.
    for account in data.get("accounts") or []:
        if isinstance(account, dict):
            for key in [k for k in account if k not in ACCOUNT_KEYS]:
                del account[key]
...
def upsert_account(email: str, **fields) -> dict | None:
    ...
    allowed = {"name", "color", "group", "region", "flagged"}
